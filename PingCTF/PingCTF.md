...
![certificate](images/pingctf.webp)
...
```
Este comando ejecuta codigo python en vim usando herramientas de sistema operativo (```import os```) cambiando nuestro usuario a id 0 (id 0 = root) (```os.setuid(0)```) ya que vim.basic tiene el bit SUID activado con dueño root. luego, con ```os.execl("/bin/sh", ...)``` reemplaza el proceso de vim por una shell, heredando privilegios root.
